...
export function CTAButton({ action, variant = 'dark', className = '', icon }: CTAButtonProps) {
  const classes = {
    dark: 'min-h-[48px] rounded-[8px] border border-[#352d27] bg-[#352d27] px-7 text-[#f3ede7] hover:bg-[#2a241f] hover:border-[#2a241f]',
    light:
      'min-h-[44px] rounded-full border border-[rgba(255,255,255,0.18)] bg-[#f3eee8] px-8 text-[#2d2621] shadow-[0_10px_30px_rgba(0,0,0,0.18)] hover:bg-[#fbf7f2]',
    'outline-dark': 'min-h-[48px] rounded-[8px] border border-[#3b332d] bg-transparent px-7 text-[#2f2924] hover:bg-[#2f2924] hover:text-[#f3ede7]',
    ...
    'dark-hero':
      'min-h-[44px] rounded-[14px] border border-[rgba(26,22,19,0.94)] bg-[rgba(26,22,19,0.92)] px-7 text-[#f4eee7] shadow-[0_10px_24px_rgba(0,0,0,0.2)] hover:bg-[rgba(18,15,13,0.96)] hover:border-[rgba(18,15,13,0.96)]',
  }

  return (
    <a
      href={action.href}
      className={`inline-flex items-center justify-center gap-3 whitespace-nowrap py-3 text-[12px] font-semibold uppercase tracking-[0.08em] transition duration-300 ${classes[variant]} ${className}`}
    >
      <span>{action.label}</span>
      {icon === 'whatsapp' && (
        <span
          aria-hidden="true"
          className="inline-flex h-5 w-5 items-center justify-center rounded-full bg-[#1f1a17] text-[#f8f4ef]"
        >
          <svg viewBox="0 0 24 24" className="h-[11px] w-[11px] fill-current" role="presentation">
