...
export function CTAButton({ action, variant = 'dark', className = '', icon }: CTAButtonProps) {
  ...
  return (
    <a
      href={action.href}
      className={`inline-flex items-center justify-center gap-3 whitespace-nowrap py-3 text-[12px] font-medium uppercase tracking-[0.12em] transition duration-300 ${classes[variant]} ${className}`}
    >
      <span>{action.label}</span>
      {icon === 'whatsapp' && (
        <span
          aria-hidden="true"
          className="inline-flex h-[22px] w-[22px] items-center justify-center rounded-full bg-[#1f1a17] text-[#f8f4ef]"
        >
