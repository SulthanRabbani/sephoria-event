...
export function CTAButton({ action, variant = 'dark', className = '', icon }: CTAButtonProps) {
  const classes = {
    dark: 'min-h-[48px] rounded-[4px] border border-[#2f2721] bg-[#2f2721] px-7 text-[#f7f2ec] hover:border-[#241d18] hover:bg-[#241d18]',
    light:
      'min-h-[48px] rounded-full border border-[rgba(255,255,255,0.28)] bg-[rgba(245,240,234,0.96)] px-8 text-[#26211d] hover:bg-white',
    'outline-dark': 'min-h-[48px] rounded-[4px] border border-[#3a312a] bg-transparent px-7 text-[#2f2924] hover:bg-[#2f2924] hover:text-[#f3ede7]',
  ...
  return (
    <a
      href={action.href}
      className={`inline-flex items-center justify-center gap-3 whitespace-nowrap py-3 text-[12px] font-medium uppercase tracking-[0.18em] transition duration-300 ${classes[variant]} ${className}`}
    >
      <span>{action.label}</span>
      {icon === 'whatsapp' && (
        <span
          aria-hidden="true"
          className="inline-flex h-[18px] w-[18px] items-center justify-center rounded-full border border-white/20 bg-[#1f1a17] text-[#fcf8f3]"
        >
